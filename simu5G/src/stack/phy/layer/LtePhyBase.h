...
class LtePhyBase : public ChannelAccess
{
  ...
  protected:
    ...
    /**
     * convoy architecture
     * Utility function to create a broadcast message for convoy control.
     */
    LteAirFrame *createConvoyControlMessage();
  ...
  public:
     ...
     */
  protected:
    MacNodeId _convoy_id;
    MacNodeId _cluster_id;
    int _convoy_direction;
    bool _convoy_full;
    bool _cluster_full;
    std::vector<MacNodeId> _publisher_id;
    std::vector<double> _publisher_x;
    std::vector<double> _publisher_y;
    std::vector<double> _publisher_fov;
    std::vector<int> _publisher_direction;
  public:
    static simsignal_t clusterBroadcastReceivedSignal;
    void setClusterBroadcastParameters(  MacNodeId convoy_id,
                                        MacNodeId cluster_id,
                                        int convoy_direction,
                                        bool convoy_full,
                                        bool cluster_full);
    void setClusterBroadcastParameters( const std::vector<MacNodeId> &publisher_id,
...
class ClusterBroadcastNotification : public omnetpp::cObject, omnetpp::noncopyable
{
  public:
    UserControlInfo *_cluster_broadcast_message;
};
