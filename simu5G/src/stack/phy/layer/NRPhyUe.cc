...
void NRPhyUe::handleAirFrame(cMessage* msg)
{
    ...
    // convoy architecture
    if (lteInfo->getFrameType() == BROADCASTPKT)
    {
        // check if the message is on another carrier frequency or handover is already in process
        if (carrierFreq != primaryChannelModel_->getCarrierFrequency())
        {
            EV << "Received broadcast packet on a different carrier frequency. Delete it." << endl;
            delete lteInfo;
            delete frame;
            return;
        }
        ...
        if (lteInfo->isNr() != isNr_)
        {
            EV << "Received broadcast packet [from NR=" << lteInfo->isNr() << "] from a different radio technology [to NR=" << isNr_ << "]. Delete it." << endl;
            delete lteInfo;
            delete frame;
            return;
        }
        broadcastHandler(frame, lteInfo);
        return;
    }
 ...
 */
void NRPhyUe::broadcastHandler(LteAirFrame* frame, UserControlInfo* lteInfo)
{
    // Step 1: Compute received signal strength
    // Step 2: Add broadcast message to record if the signal strength exceeds the minimum threshold
    frame->setControlInfo(lteInfo);
    double rssi;

    if (((getNodeTypeById(lteInfo->getSourceId()) == ENODEB) || (getNodeTypeById(lteInfo->getSourceId()) == GNODEB)) && lteInfo->getSourceId() == masterId_)
    {
    ...
    if (rssi < minRssi_)
    {
        EV << "Signal too weak - minRssi[" << minRssi_ << "]" << endl;
        delete frame;
        return;
    }

    lteInfo->setRssi(rssi);

    // Emit broadcast-received signal for ccs agent
    ClusterBroadcastNotification notification_msg;
    notification_msg._cluster_broadcast_message = lteInfo;
    emit(clusterBroadcastReceivedSignal, &notification_msg);

    delete frame;
}

void NRPhyUe::attachToCluster(MacNodeId mac_cell_id, double rssi, bool is_rsu)
{
    Enter_Method_Silent();
    forceHandover(mac_cell_id, rssi);

    // Setup cluster status reporting
    convoyCtrlUnicastInterval_ = par("convoyCtrlUnicastInterval").doubleValue();
    if (convoyCtrlUnicastInterval_ != 0)
    {
        // self message to trigger convoy control unicast message
        convoyCtrlUnicastStarter_ = new cMessage("convoyCtrlUnicastStarter");
        scheduleAt(NOW + convoyCtrlUnicastInterval_, convoyCtrlUnicastStarter_);
    }
}

void NRPhyUe::handleSelfMessage(cMessage *msg)
{
    omnetpp::simtime_t current_time = omnetpp::simTime();
    if (msg->isName("convoyCtrlUnicastStarter"))
    {
        if(!binder_->hasUeHandoverTriggered(nodeId_))
        {
            if(masterId_ == 0)
                return;
            // send convoy control broadcast message
            EV_INFO << current_time << " - NRPhyUe::handleSelfMessage(): sending cluster control message to " << masterId_ << std::endl;
            LteAirFrame *f = createConvoyControlMessage();
            UserControlInfo *control_info = check_and_cast<UserControlInfo *>(f->getControlInfo());
            control_info->setDirection(UL);
            control_info->setDestId(masterId_);
            control_info->setIsBroadcast(false);
            sendUnicast(f);
        }
        scheduleAt(NOW + convoyCtrlUnicastInterval_, msg);
    }
    else
