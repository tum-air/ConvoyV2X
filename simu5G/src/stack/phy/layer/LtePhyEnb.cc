...
LtePhyEnb::LtePhyEnb()
{
    das_ = nullptr;
    bdcStarter_ = nullptr;
    convoyCtrlBroadcastStarter_ = nullptr;
}

LtePhyEnb::~LtePhyEnb()
{
    cancelAndDelete(bdcStarter_);
    cancelAndDelete(convoyCtrlBroadcastStarter_);
...
void LtePhyEnb::initialize(int stage)
{
    ...
    else if (stage == 1)
    {
        ...
        /* convoy architecture */
        convoyCtrlBroadcastInterval_ = cellInfo_->par("convoyCtrlBroadcastInterval");
        if (convoyCtrlBroadcastInterval_ != 0) {
            // self message to trigger convoy control broadcast message
            convoyCtrlBroadcastStarter_ = new cMessage("convoyCtrlBroadcastStarter");
            scheduleAt(NOW, convoyCtrlBroadcastStarter_);
        }
    }
...
void LtePhyEnb::handleSelfMessage(cMessage *msg)
{
    ...
    else if (msg->isName("convoyCtrlBroadcastStarter"))
    {
        // send convoy control broadcast message
        LteAirFrame *f = createConvoyControlMessage();
        sendBroadcast(f);
        scheduleAt(NOW + convoyCtrlBroadcastInterval_, msg);
    }
    else
    {
        delete msg;
...
bool LtePhyEnb::handleControlPkt(UserControlInfo* lteinfo, LteAirFrame* frame)
{
    ...
    //handle convoy control pkt
    if (lteinfo->getFrameType() == BROADCASTPKT)
    {
        handleBroadcastPkt(lteinfo, frame);
        delete frame;
        return true;
    }
    return false;
 ...
 * Convoy architecture
 */
void LtePhyEnb::handleBroadcastPkt(UserControlInfo* lteinfo, LteAirFrame *frame)
{
