 ...
 * Convoy architecture
 */
void Binder::notifyConvoy(int id)
{
    // Add to list if new, also initialize cluster map if not done already
    std::vector<int>::iterator ref_entry = find (_id_list_convoys.begin(), _id_list_convoys.end(), id);
    if (ref_entry != _id_list_convoys.end())
        return;
    _id_list_convoys.push_back(id);

    std::vector<MacNodeId> cluster_data;
    _cluster_map.insert(std::pair<int, std::vector<MacNodeId>>(id, cluster_data));
}
void Binder::deNotifyConvoy(int id)
{
    _id_list_convoys.erase(std::remove(_id_list_convoys.begin(), _id_list_convoys.end(), id), _id_list_convoys.end());
}
int Binder::getMaxIdConvoy()
{
    int max_id = -1;
    if(_id_list_convoys.size() > 0)
        max_id = *(std::max_element(_id_list_convoys.begin(), _id_list_convoys.end()));
    return(max_id);
}
int Binder::getMaxIdCluster(int convoy_id)
{
    // Get max no.of clusters if convoy exists else return -1
    int max_id = -1;
    std::vector<int>::iterator ref_entry = find (_id_list_convoys.begin(), _id_list_convoys.end(), convoy_id);
    if (ref_entry != _id_list_convoys.end())
        max_id = _cluster_map[convoy_id].size() - 1;
    return(max_id);
}
void Binder::notifyCluster(int convoy_id, int cluster_id, MacNodeId cluster_manager)
{
    while(_cluster_map[convoy_id].size() < (cluster_id + 1))
        _cluster_map[convoy_id].push_back(0);
    _cluster_map[convoy_id].at(cluster_id) = cluster_manager;
}
void Binder::deNotifyCluster(MacNodeId cluster_manager)
{
    // Iterate through all maps entries and remove cluster_manager
    for (auto it : _cluster_map)
    {
        std::vector<MacNodeId>::iterator ref_entry = find (it.second.begin(), it.second.end(), cluster_manager);
        if (ref_entry != it.second.end())
            it.second.erase(std::remove(it.second.begin(), it.second.end(), cluster_manager), it.second.end());
    }
}
MacNodeId Binder::getManagerId(int convoy_id, int cluster_id)
{
    return(_cluster_map[convoy_id].at(cluster_id));
}
int Binder::getClusterId(int convoy_id, MacNodeId cluster_manager)
{
    int cluster_id = -1;
    std::vector<int>::iterator ref_entry_convoy_id = find (_id_list_convoys.begin(), _id_list_convoys.end(), convoy_id);
    if (ref_entry_convoy_id != _id_list_convoys.end())
    {
        std::vector<MacNodeId>::iterator ref_entry_cluster_manager = find (_cluster_map[convoy_id].begin(), _cluster_map[convoy_id].end(), cluster_manager);
        if (ref_entry_cluster_manager != _cluster_map[convoy_id].end())
            cluster_id = std::distance(_cluster_map[convoy_id].begin(), ref_entry_cluster_manager);
    }
